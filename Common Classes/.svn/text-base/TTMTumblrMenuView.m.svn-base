...
#define CHTumblrMenuViewImageHeight 65
...
#define TumblrBlue [UIColor colorWithRed:16/255.0f green:16/255.0f blue:16/255.0f alpha:1.0]

@interface TTMTumblrMenuItemButton : UIControl
- (id)initWithTitle:(NSString*)title andIcon:(UIImage*)icon andSelectedBlock:(TTMTumblrMenuViewSelectedBlock)block;
@property(nonatomic,copy)TTMTumblrMenuViewSelectedBlock selectedBlock;
@end

@implementation TTMTumblrMenuItemButton
{
...
- (id)initWithTitle:(NSString*)title andIcon:(UIImage*)icon andSelectedBlock:(TTMTumblrMenuViewSelectedBlock)block
{
    ...
    if (self) {
        ...
        titleLabel_.font = [UIFont fontWithName:LotoLight size:10.0f];
...
@implementation TTMTumblrMenuView
{
...
{
    TTMTumblrMenuItemButton *button = [[TTMTumblrMenuItemButton alloc] initWithTitle:title andIcon:icon andSelectedBlock:block];
...
- (void)layoutSubviews
{
    ...
    for (NSUInteger i = 0; i < buttons_.count; i++) {
        TTMTumblrMenuItemButton *button = buttons_[i];
...
{
    if ([gestureRecognizer.view isKindOfClass:[TTMTumblrMenuItemButton class]]) {
        return NO;
    }
...
- (void)buttonTapped:(TTMTumblrMenuItemButton*)btn
{
    //[self dismiss:nil];
    double delayInSeconds = CHTumblrMenuViewAnimationTime  + CHTumblrMenuViewAnimationInterval * (buttons_.count + 0.1);
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        btn.selectedBlock();
...
- (void)riseAnimation
{
    ...
    for (NSUInteger index = 0; index < buttons_.count; index++) {
        TTMTumblrMenuItemButton *button = buttons_[index];
...
- (void)show
{
    ...
    self.frame = CGRectMake(0, 55, topViewController.view.frame.size.width, 200);
